...
from vcat.core.event_bus import EventBus
...
from vcat.core.obj import (
    ...
    CancelRequest,
    SubscribeRequest,
    DivMetaInfo, TagMetaInfo, MetaObj

)
from vcat.config import ROOT_PATH, conf
...
# 委托状态映射
STATUS_CTP2VT: Dict[str, OrderStatus] = {
    THOST_FTDC_OST_NoTradeQueueing: OrderStatus.SUBMIT,
    THOST_FTDC_OST_PartTradedQueueing: OrderStatus.PART_TRADED,
    THOST_FTDC_OST_AllTraded: OrderStatus.ALL_TRADED,
    THOST_FTDC_OST_Canceled: OrderStatus.ALL_CANCELLED,
    THOST_FTDC_OST_Unknown: OrderStatus.SUBMIT
}
...
ORDERTYPE_VT2CTP: Dict[OrderType, tuple] = {
    OrderType.LMT: (THOST_FTDC_OPT_LimitPrice, THOST_FTDC_TC_GFD, THOST_FTDC_VC_AV),
    OrderType.MKT: (THOST_FTDC_OPT_AnyPrice, THOST_FTDC_TC_GFD, THOST_FTDC_VC_AV),
...
# 产品类型映射
PRODUCT_CTP2VT: Dict[str, ContractType] = {
    THOST_FTDC_PC_Futures: ContractType.FUT,
    THOST_FTDC_PC_Options: ContractType.OPT,
    THOST_FTDC_PC_SpotOption: ContractType.OPT,
    THOST_FTDC_PC_Combination: ContractType.SPREAD
}
...
class CtpGateway(BaseGateway):
        ...
        def meta(self):
            _meta = DivMetaInfo(
                ...
                headers=[
                    ...
                    TagMetaInfo(name='经纪商代码', field='brokerid', value=self.brokerid),
                    TagMetaInfo(name='交易服务器', field='td_address', value=self.td_address),
                    TagMetaInfo(name='行情服务器', field='md_address', value=self.md_address),
                    TagMetaInfo(name='产品名称', field='appid', value=self.appid),
                    TagMetaInfo(name='授权编码', field='auth_code', value=self.auth_code)
                ]
            )
    ...
    def connect(self, setting: dict) -> None:
        ...
        self.config_setting.td_address = td_address = setting["td_address"]
        self.config_setting.md_address = md_address = setting["md_address"]
        self.config_setting.appid = appid = setting["appid"]
        self.config_setting.auth_code = auth_code = setting["auth_code"]
    ...
    def init_query(self) -> None:
        ...
        self.engine.guard_left.event_bus.as_handler(
            category=HandlerCategory.EVENT,
...
class CtpMdApi(MdApi):
    ...
    def connect(self, address: str, userid: str, password: str, brokerid: str) -> None:
        ...
        if not self.connect_status:
            path: Path = os.path.join(conf.CUSTOM_DIR, self.gateway_name.lower())
    ...
    def subscribe(self, req: SubscribeRequest) -> None:
        ...
        if self.login_status:
            self.subscribeMarketData(req.code)
        self.subscribed.add(req.code)
...
class CtpTdApi(TdApi):
    ...
    def onRspQryInstrument(self, data: dict, error: dict, reqid: int, last: bool) -> None:
        ...
        if contract_type:
            ...
            symbol_contract_map[contract.code] = contract
    ...
    def onRtnOrder(self, data: dict) -> None:
        ...
        order: OrderData = OrderData(
            ...
            exchange=contract.exchange,
            t_order_id=orderid,
            order_type=ORDERTYPE_CTP2VT[tp],
            direction=DIRECTION_CTP2VT[data["Direction"]],
